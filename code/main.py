...
def read_container_csv(file: str) -> list[object]:
    with open(
        file, newline=""
    ) as csv_file:  # I decided to use csv first, since I don't know the final database structure and I'd rather just code right now instead of contemplating my containers' structure
        ...
        for item in container:
            ...
            ):  # default in dict.get() doesn't work if None is already set (it only works when index is not set)
                item["quantity"] = 1
            item_object = Item(
                item.get("name"), item.get("quantity"), item.get("subitems")
            )
...
def inventory():
    ...
    return render_template(
        "main.html", rooms=rooms_list
    )  # rooms contain all other objects
